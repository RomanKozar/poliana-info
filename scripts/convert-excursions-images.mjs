/**
 * Конвертує PNG/HEIC/JPEG у WebP і стискає WebP у public/images/excursions/{mountains,ATV}.
 */
import fs from 'fs'
...
const TARGETS = [
	{ subdir: path.join('excursions', 'mountains'), maxWidth: 1200, maxHeight: 900, quality: 82 },
	{ subdir: path.join('excursions', 'ATV'), maxWidth: 1200, maxHeight: 900, quality: 82 },
]
...
async function processDir(target) {
	...
	const label = target.subdir.replace(/\\/g, '/')
	if (!fs.existsSync(dir)) {
		console.warn('skip (no dir):', label)
		return
	}
