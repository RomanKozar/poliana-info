/**
 * Картки «Піші стежки Поляни»: paths-*.jpg → paths-*.webp
 * Вхід:  public/images/excursions/mountains/paths/*
 * Вихід: public/images/excursions/mountains/paths/paths-<n>.webp
 */
import fs from 'fs'
...
const dir = path.join(__dirname, '..', 'public', 'images', 'excursions', 'mountains', 'paths')
...
function numFromName(file) {
	const m = file.match(/paths-(\d+)/i) ?? file.match(/(\d+)/)
...
async function main() {
	...
	if (!files.length) {
		console.log('Немає JPG/PNG/HEIC у paths/.')
		return
	}
	...
	for (const { f, n } of toProcess) {
		...
		const outPath = path.join(dir, `paths-${n}.webp`)
