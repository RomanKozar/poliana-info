...
import VedmezhaRouteMap from '@/components/excursions/VedmezhaRouteMap'
import {
...
export default function VedmezhaDetailPage() {
	return (
		<PhotoGalleryProvider images={VEDMEZHA_PHOTOS}>
			<div className='flex min-h-0 w-full flex-col bg-[#F5F6F7] pb-3 sm:pb-12'>
				<section className='border-b border-slate-200/80 bg-white px-4 py-8 sm:px-16 lg:py-12 lg:px-24'>
					<div className='grid items-start gap-8 lg:grid-cols-2 lg:gap-14'>
						<div className='max-w-xl lg:max-w-none'>
							...
							<h1 className='mt-2 text-3xl font-bold text-[#2D333D] sm:text-4xl'>Гора Ведмежа</h1>
							...
							<div className='grid gap-4 sm:grid-cols-2'>
								...
								<div className='rounded-2xl border border-slate-200/90 bg-[#F5F6F7] p-5 shadow-sm'>
									...
									<p className='mt-2 text-lg font-bold text-[#2D333D]'>Орієнтовно 2–4 години</p>
							...
							<div className='mt-5 grid gap-4 sm:grid-cols-2'>
								<div className='rounded-2xl border border-slate-200/90 bg-white p-5 shadow-sm'>
									...
									<p className='mt-2 text-sm text-slate-600'>
										Підкаже оптимальний маршрут і точки для фото, допоможе з безпекою на схилах.
									</p>
								</div>
								<div className='rounded-2xl border border-slate-200/90 bg-white p-5 shadow-sm'>
									<p className='flex items-center gap-2 text-xs font-semibold uppercase tracking-wide text-slate-500'>
						...
						<div>
							...
							<div className='px-4 py-4 sm:px-5'>
								<p className='text-sm font-semibold text-[#2D333D]'>Ліс + відкриті схили</p>
								<p className='mt-1 text-sm text-slate-600'>Контрастний маршрут і гарні панорами поруч із Поляною.</p>
							</div>
						</div>
					</div>
				</section>
				...
				<section
					...
					aria-labelledby='vedmezha-gallery-heading'
				>
					<h2 id='vedmezha-gallery-heading' className='text-xl font-bold text-[#2D333D] sm:text-2xl'>
						Фото
					</h2>
					<p className='mt-2 max-w-3xl text-sm text-slate-600'>Натисніть, щоб переглянути та гортати.</p>
					<div className='mt-6'>
						<PhotoGalleryStrip images={VEDMEZHA_PHOTOS} />
					</div>
				</section>
