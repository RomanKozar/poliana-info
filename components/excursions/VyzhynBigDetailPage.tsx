...
import VyzhynBigRouteMap from '@/components/excursions/VyzhynBigRouteMap'
import {
...
const VYZHYN_BIG_PHOTOS = summitDetailGalleryImages(3)

export default function VyzhynBigDetailPage() {
	return (
		<PhotoGalleryProvider images={VYZHYN_BIG_PHOTOS}>
			<div className='flex min-h-0 w-full flex-col bg-[#F5F6F7] pb-3 sm:pb-12'>
				<section className='border-b border-slate-200/80 bg-white px-4 py-8 sm:px-16 lg:py-12 lg:px-24'>
					<div className='grid items-start gap-8 lg:grid-cols-2 lg:gap-14'>
						<div className='max-w-xl lg:max-w-none'>
							...
							<h1 className='mt-2 text-3xl font-bold text-[#2D333D] sm:text-4xl'>Гора Великий Вижень</h1>
							...
							<p className='mt-4 leading-relaxed text-slate-600'>
								Більш виразний підйом для підготовлених туристів. Рекомендуємо взяти воду, зручне взуття та
								перевірити прогноз перед виходом.
							</p>
							...
							<div className='grid gap-4 sm:grid-cols-2'>
								...
								<div className='rounded-2xl border border-slate-200/90 bg-[#F5F6F7] p-5 shadow-sm'>
									...
									<p className='mt-2 text-lg font-bold text-[#2D333D]'>Орієнтовно 3–5 годин</p>
							...
							<div className='mt-5 grid gap-4 sm:grid-cols-2'>
								<div className='rounded-2xl border border-slate-200/90 bg-white p-5 shadow-sm'>
									...
									<p className='mt-2 text-sm text-slate-600'>
										Підбір маршруту під вашу підготовку та контроль безпеки на підйомі.
									</p>
								</div>
								<div className='rounded-2xl border border-slate-200/90 bg-white p-5 shadow-sm'>
									<p className='flex items-center gap-2 text-xs font-semibold uppercase tracking-wide text-slate-500'>
						...
						<div>
							<HeroGalleryImage
								src={VYZHYN_BIG_PHOTOS[0]}
								alt='Гора Великий Вижень'
								priority
							...
							<div className='px-4 py-4 sm:px-5'>
								<p className='text-sm font-semibold text-[#2D333D]'>Для підготовлених туристів</p>
								<p className='mt-1 text-sm text-slate-600'>
									Виразніший підйом і панорами — краще планувати з запасом часу.
								</p>
							</div>
						</div>
					</div>
				...
				<section
					...
					aria-labelledby='vyzhyn-big-gallery-heading'
				>
					<h2 id='vyzhyn-big-gallery-heading' className='text-xl font-bold text-[#2D333D] sm:text-2xl'>
						Фото
					</h2>
					<p className='mt-2 max-w-3xl text-sm text-slate-600'>Натисніть на фото, щоб переглянути та гортати.</p>
					<div className='mt-6'>
						<PhotoGalleryStrip images={VYZHYN_BIG_PHOTOS} />
					</div>
				</section>
